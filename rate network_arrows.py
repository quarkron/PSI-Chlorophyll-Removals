...
for i in range(N):
    for j in range(N):
        if i > j:
            ...
            cen_jprime = ((R-1.5*r)/R)*(cen_j - cen_i) + cen_i
            cen_iprime = ((R-1.5*r)/R)*(cen_i - cen_j) + cen_j
            if rate_ij > rate_ji:
                if rate_ij > cutoff:
                    rate_solids.addArrow(cen_i, cen_jprime, rate_ij/scaler, colorByName("deep sky blue"))
            if rate_ij == rate_ji:
                if rate_ij > cutoff:
                    rate_solids.addArrow(cen_iprime, cen_jprime, rate_ij/scaler, colorByName("deep sky blue"))
                    rate_solids.addArrow(cen_jprime, cen_iprime, rate_ji/scaler, colorByName("deep sky blue"))
            if rate_ij < rate_ji:
                if rate_ji > cutoff:
                    rate_solids.addArrow(cen_j, cen_iprime, rate_ji/scaler, colorByName("deep sky blue"))
            if rate_ij > 10:
                key_chl.add(i)
                key_chl.add(j)
...
for i in range(N):
    ...
    if nodeName in ['CLA1012.B', 'CLA1013.A', 'CLA1022.A', 'CLA1023.B']:
        ...
        rate_solids.addSphere(cen_i, r, colorByName('green'))
